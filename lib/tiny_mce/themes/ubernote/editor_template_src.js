...
(function(tinymce) {
	...
	tinymce.ThemeManager.requireLangPack('ubernote');

	tinymce.create('tinymce.themes.UbernoteTheme', {
		...
		_createForeColorMenu : function() {
			...
			if (s.theme_advanced_more_colors) {
				o.more_colors_func = function() {
					t._mceColorPicker(0, {
						color : t.editor.last_fg_color,
						func : function(co) {
						    t.editor.last_fg_color = co;
							c.setColor(co);
						}
					});
				};
		...
		_createBackColorMenu : function() {
			...
			if (s.theme_advanced_more_colors) {
				o.more_colors_func = function() {
					t._mceColorPicker(0, {
						color : t.editor.last_bg_color,
						func : function(co) {
						    t.editor.last_bg_color = co;
						    c.setColor(co);
						}
					});
				};
		...
		renderUI : function(o) {
			...
			if (DOM.get(s.theme_advanced_toolbar_container))
			    ...
			    if( s.content_editable ) {   
			        var lo = s.theme_advanced_toolbar_location;
			        if( lo == 'top' ) {
			            var editor = DOM.get(n);
			            editor.parentNode.insertBefore(p, editor);
			        } else if( lo == 'bottom' )
			            DOM.insertAfter(p, n);
			    }
			    else {
...
*/
		...
		_simpleLayout : function(s, tb, o, p) {
			...
			if (s.readonly) {
				n = DOM.add(tb, 'tr');
                n = ic = DOM.add(n, 'td', {'class' : 'mceIframeContainer', height: o.height - 29});
				return ic;
			}
			...
			if (!s.theme_advanced_toolbar_container) {
				n = DOM.add(tb, 'tr', {width: '100%'});
				// XXX this 29 pixels height is assuming there is one scroll bar.  Added by shane.
				//  We add the height here to keep small notes from resizing from a larger size to a
				//  smaller size whenever the resizeToContent is called.
				n = ic = DOM.add(n, 'td', {'class' : 'mceIframeContainer', height: o.height - 29, width: '100%'});
		    }
...
*/
		...
		_mceBackColor : function() {
			...
			this._mceColorPicker(0, {
				...
				func : function(co) {
				    t.editor.last_bg_color = co;
				    if( t.editor.selection.getContent() )
				    {
	...
	tinymce.ThemeManager.add('ubernote', tinymce.themes.UbernoteTheme);
}(tinymce));