...
function CollapsingNumberListPlugin()
{
    this.m_nSize = undefined;
    this.m_nCurrentSelection = undefined;

    CollapsingNumberListPlugin.Base.constructor.apply( this );
}
UberObject.Base( CollapsingNumberListPlugin, Plugin );

Object.extend( CollapsingNumberListPlugin.prototype, {
    loadConfigParams: function()
    {
        var objConfigParams = {
            ...
            m_strNewSelectionMessage: { type: 'string', bRequired: false, default_value: 'CollapsingNumberListNewSelection' },
            m_strListSelector: { type: 'string', bRequired: false, default_value: 'NumberList' }
        };

        CollapsingNumberListPlugin.Base.loadConfigParams.apply( this );
        Object.extend( this.m_objConfigParams, objConfigParams );
    },

    RegisterMessageHandlers: function()
    {
        this.RegisterListener( 'setsize', this.setSize, this )
            .RegisterListener( 'setselection', this.setSelection, this );
    },
    ...
    */
    _clear: function()
    {
        ...
        for( var nIndex = 0, objElement;
            objElement = objPlugged.$( nIndex.toString() + this.m_strMessagingID ); 
            ++nIndex )
        {
            if( objElement._button )
            {    // Not all of the elements are buttons.
                objPlugged.detachButton( objElement, this.OnNumberSelection );
            } // end if
            
            objPlugged.detachHTMLElement( nIndex.toString() + this.m_strMessagingID );
    ...
    */
    _create: function( in_nSelection )
    {
        ...
        var objPlugged = this.getPlugged();
        var objListElement = objPlugged.$( this.m_strListSelector );
        ...
        for( var nIndex = 0, nSize = anSet.length, nElementNum = 0; nIndex < nSize; nIndex++ )
        {
            ...
            var strID = nElementNum.toString() + this.m_strMessagingID;
            ...
            {   // If there is a gap in numbers, put in a ...
                ...
                objPlugged.attachHTMLElement( strID, objElement );
                nElementNum++;            
            } // end if
            var strID = nElementNum.toString() + this.m_strMessagingID;
            ...
            var bSelected = nValue == in_nSelection;
            objElement.innerHTML = this._getNumberString( nValue, bSelected );
			if( bSelected )
			{
				objElement.addClassName( 'selected' );
			} // end if
            ...
            objElement.title = this.m_strTooltipText + ( nValue + 1 ).toString();
            
            objPlugged.attachHTMLElement( strID, objElement );
            objPlugged.attachButton( objElement, this.OnNumberSelection, this, false );
            nElementNum++;
    ...
    */
    setSelection: function( in_nIndex, in_bForce )
    {
        ...
        {
            // This function is called from the onclick event handler, and there may
            //  be handlers higher up the DOM tree listening for click events as well.
            //  If the element that this DOM event is removed before all of the handlers
            //  get their turn, some event handlers will not get run.  An example of this
            //  is the onclick handler on the body in GlobalEventHandler.  It closes
            //  all of the popup menus whenever there is a click somehwere in the body.
            ...
            {
                this.RaiseForAddress( this.m_strNewSelectionMessage, 
                    this.getPlugged().m_strMessagingID, [ in_nIndex ] );
            } // end if
