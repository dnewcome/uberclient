Accordion = function()
{
    ...
    this.strMessagingID = Messages.generateID();
};
...
Object.extend( Accordion.prototype, {    
    ...
    */
    init: function( in_strPrefix )
    {
            ...
            objElement = document.getElementById( in_strPrefix + nIndex + "-header" ); ++nIndex )
        {
            ...
            Messages.RegisterListener( 'onclick', objElement, this.strMessagingID, 
                this.OnHeaderClicked, this );

	        if( ! BrowserInfo.ie && ! Nifty.csscorners )
	        {
                Nifty( 'div#' + objElement.id, 'top' );
            } // end if
        } // end for
    },
    ...
    */
    _processElements: function( in_nIndexToOpen, in_strPostfix, in_strClassName )
    {
        ...
        if( !BrowserInfo.ie && !Nifty.csscorners && '-header' == in_strPostfix )
        {
            objOpenElement && Timeout.setTimeout( Nifty, 0, window, [ 'div#' + strElementToOpenID, 'top' ] );
            objCloseElement && Timeout.setTimeout( Nifty, 0, window, [ 'div#' + strElementToCloseID, 'top' ] );
        }               
